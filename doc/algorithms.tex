...
sign as~$d$ and compute as in the case $x<0$ real and $y$ an integer above
that
\[
\Re (z)
= R
\cos \left( 2 c \sign b (\pi / 2)
+  \big( c \tilde \delta_b
...
Assume that $2c \sign b$, which is an integer, is congruent to~$1$
...
and either $|a| < 1$ and $\sign (d) = -1$,
    or     $|a| > 1$ and $\sign (d) = +1$,
    or      $a = -1$.
When $b$ and $c$ have opposite signs, we have $2 c \sign b = - 2 |c|$,
and the congruence condition becomes that $2 |c| \equiv 3 \pmod 4$.

If $2c \sign b$ is congruent to~$3$ modulo~$4$, then the cosine function
